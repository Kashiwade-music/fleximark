...
- cargo test --workspace --all-targets: 144 tests passed。fleximark-plugin-hostは既存20件と追加characterization 5件の計25件が成功した。
- Node pure RPC/preview/protocol tests: 54 tests passed、VS Code Electron tests: 75 tests passed、Python unittest discovery: 71 tests discovered（WindowsではPOSIX executable mode test 1件をskip）。
- cargo fmt、cargo clippy、TypeScript noEmit、ESLint、architecture 56 capabilities、production browser/extension build、performance budget、VSIX内容検査を含む `mise run verify` が成功した。
- `fleximark-plugin-host/src/lib.rs` は2,550行から13行のprivate module宣言と明示re-exportだけのfacadeになった。runtime/wasmtime、package、pipeline、edit_map、candidate、errorは一方向に依存し、module cycleとcrate-rootへの逆依存はない。
...
- cargo test --workspace --all-targets: 150 tests passed。Node pure RPC/preview/protocol/supervisor testsは68件、VS Code Electron testsは79件、Python unittest discoveryは71件（WindowsではPOSIX executable mode test 1件をskip）が成功した。
- TypeScript noEmit、ESLint、Prettier、architecture 56 capabilities、production browser/extension build、performance budget、VSIX内容検査を含む `mise run verify` が成功した。
- `adapter.mts` は1,374行から1,175行になり、release manifest、workspace selection、position conversion、error/redaction policyをpure moduleへ移した。既存9 exportsとFlexiMarkAdapterのpublic API、command/status/user-visible textは維持した。
- framework非依存の `DaemonSupervisor` がprocess/RPC identity、connection generation、concurrent ensure、workspace revision、restart timer、250/500/1000/2000/4000ms backoff、5回上限、30秒超過reset、recovery status、stop/disposeを所有する。spawn/clock/timer/RPC/log/status/replayを注入し、raceを決定的なfakeで検証する。
- launchはbinary resolve→spawn/bind→protocol initialize→workspace initialize→document replay→preview replay、shutdownはrequest→exit notification→RPC close→process killの順を維持する。dispose/pending resolve、deferred exit、late generation、workspace revision中のreplay、preview replay中exit、dialogのretry/open-output/dismiss/rejectをtest化した。
...
- cargo test --workspace --all-targets: 150 tests passed。Node pure RPC/preview/protocol/coordinator testsは162件、VS Code Electron testsは114件、Python unittest discoveryは71件（WindowsではPOSIX executable mode test 1件をskip）が成功した。
- TypeScript noEmit、ESLint、Prettier、architecture 56 capabilities、production browser/extension build、performance budget、VSIX内容検査を含む `mise run verify` が成功した。
- `web/preview-client/index.mts` は511行から186行、`enhance.mts` は413行から115行になった。前者はlive DOM/state commit facade、後者はgeneration/audio/fingerprint/tabs coordinatorとして残し、content/security、asset staging、patch transaction、host async task、browser navigation transportと7種のfeature rendererを一方向のprivate moduleへ分離した。
- snapshot/patchのatomic clone-commit、単一sourceのpatch属性allowlist、1 MiB/8 MiB asset境界、staged/owned blob URL revoke、host batch、VS Code即時ACK、browser SSE再接続とnavigation POSTをcharacterizationした。stale Mermaid、dispose後enhance、invalid batch後enhance、途中URL生成失敗、audio init/prime/start/stop/close失敗を他blockとlive stateから隔離した。
- 4巡の実装と各巡2名の独立レビュー後に未解決P0〜P3はない。実audio runtimeはcontext/oscillatorを生成直後から所有し、同期throw・非同期reject・pending coalesceを上位trackerへ伝播して、成功時だけ解放し失敗時は後続disposeで再試行する。
- FORM/actionとCSP `form-action`、Mermaid SVG sanitizer依存、asset contentHash/CSS fingerprintの再計算、VS Code message source/originは既存security debtとして挙動を変えず、Phase 12後の独立security fixへ残す。asset二重atobは別commit、identityMap最適化はbenchmark後という計画を維持した。

### Phase 12 完了時のベースライン

- `cargo test --workspace --all-targets --locked`: 150 tests passed。Node pure testsは162件、非重複のVS Code Electron integration testsは38件、Python unittest discoveryは91件（WindowsではPOSIX executable mode test 1件をskip）が成功した。
- TypeScriptはsolution/base/browser/adapter/unit/Electronのproject reference境界とし、全61 `.mts` sourceの直接ownerを1つに固定した。正規入口の `yarn exec tsc -b`、ESLint、Prettier、architecture 56 capabilities、production build、performance budget、pure/Electron独立gate、VSIX内容検査を含む `mise run verify` が成功した。
- test artifactを `out/test/unit` と `out/test/electron` へ分離し、releaseもpure gateを明示実行する。multi-root、audio start、navigation debounceの固定sleepはbounded predicate、deferred signal、fake clockへ変え、timeout・exceptionでもtimerをsingle-settle cleanupする契約をtest化した。
- Rust 1.85のroot workspaceを6 host platform/architectureでcheckし、独立fixture workspaceは `wasm32-wasip2` でもcheckする。Rust 1.85で使えないlet-chainを同値なnested `if`へ変換し、root/fixture lockをMSRV互換版へ固定した。現行toolchainのfmt/test/clippyと配布binary buildは分離したままである。
- CLIの未使用direct parser dependencyを削除し、daemonのbase64をworkspace継承へ統一した。operational Cargo入口は`--locked`を使い、rootとplugin fixtureの両workspaceをDependabot対象にした。
- actual VSIX verifierはcanonical path、CRC、size/type、許可layout、両preview host、6 daemon/manifest、package `main`/iconと全contribution objectを検証する。CIはpackage後・upload前にこの同一validatorを実行する。
- l10nの`.mts`/`.cts`は原本を変えずtemporary treeのcopyだけを`.ts`へ変換する。`capabilities/v0.16.14-inventory.json`は当該tagとblob SHAが一致する歴史audit baselineとして保持し、現行architecture gateの入力にはしない。
- 7巡の実装と複数回・複数名の独立レビューで、fixture MSRV、test wait、CLI option、VSIX allowlist/宣言照合、Windows reserved path、JSON scalar型をadversarialに補強した。新規の未解決P0〜P3はない。
- test-only `FlexiMarkTestApi`のmodule-global ownershipは既存compatibilityとして維持した。HTTP header/URI containment、FORM/CSP、VS Code message originなど先行Phaseで記録したsecurity debtは、本リファクタと分けた将来のsecurity changeとする。
...
- Phase 12の再調査で `PreviewState.url` だけがwrite-onlyと判明し、削除した。`DocumentState.version` と `WorkspaceRuntime.removed` は分割後のcoordinator/currentness gateから読まれるため保持する。
- #checkpointの未使用runtime引数はPhase 10のdocument coordinator抽出時に解消した。
- SessionRegistry::configure_plugins等のpublic symbolはrepo内参照がなくても外部利用が不明なので削除しない。
- capabilities/v0.16.14-inventory.jsonはrepo内の現行gateからは参照されないが、当該tagとblob SHAが一致する歴史audit baselineであるため保持する。
...
- 具体的な変更:
  ...
  - create-preview RPC待機中のdispose/remove/generation変更をcommit前に再検証し、late応答がpanelまたは外部URLを生成しないようにする。
...
- 状態: 2026-09-12 完了。DOM/security、asset、patch、feature renderer、host/transportをprivate ownerへ分離し、27件の永続的なcharacterization/regression testと4巡の複数独立レビューでatomic commit、stale async、blob/audio cleanupを検証した後、`mise run verify` に成功した。
- 優先度 / ROI: P1 / 中〜高。
...
### Phase 12 — Test/build境界、dependency hygiene、低リスクcleanup（完了）
...
TypeScriptの正規入口はyarn exec tsc -bである。
...
Phase 0は2026-09-11、Phase 1〜12は2026-09-12に完了した。この計画の全Phaseは、Phaseを混在させず固定した外部挙動とfull gateを維持して完了している。今後のsecurity・performance・clean breakは、記録済みの残余項目ごとに独立した変更として扱う。
