...
エクスポートは文書モデルから Portable 用 HTML を生成し、ローカルアセットを解決し、テーマと共通ブラウザランタイムを組み込む。既定の出力先は文書の隣の `<文書名>.fleximark-export` ディレクトリである。生 HTML はプレビューとエクスポートの双方で共通の安全化処理を通り、通常の文書要素と属性を保ったまま実行・埋め込み・送信機能を除去する。
